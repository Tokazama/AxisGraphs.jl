...
@testset "SimpleWeightedGraphs" begin
    ...
    @test @inferred(eltype(AxisIndicesGraph(SimpleWeightedGraph()))) == Int
    @test @inferred(eltype(AxisIndicesGraph(SimpleWeightedGraph(adjmx1)))) == Int

    @test @inferred(ne(AxisIndicesGraph(SimpleWeightedGraph(path_digraph(5))))) == 4
    @test @inferred(!is_directed(AxisIndicesGraph(SimpleWeightedGraph())))

    @test @inferred(eltype(AxisIndicesGraph(SimpleWeightedDiGraph()))) == Int
    @test @inferred(eltype(AxisIndicesGraph(SimpleWeightedDiGraph(adjmx2)))) == Int
    @test @inferred(ne(AxisIndicesGraph(SimpleWeightedDiGraph(path_graph(5))))) == 8
    @test @inferred(is_directed(AxisIndicesGraph(SimpleWeightedDiGraph)))


    for gbig in [AxisIndicesGraph(SimpleWeightedGraph(0xff)), AxisIndicesGraph(SimpleWeightedDiGraph(0xff))]
        @test @inferred(!add_vertex!(gbig))    # overflow
        @test @inferred(!add_vertices!(gbig, 10))
    end

    gdx = AxisIndicesGraph(SimpleWeightedDiGraph(path_digraph(4)))
    gx = AxisIndicesGraph(SimpleWeightedGraph())
    ...
    gx = AxisIndicesGraph(SimpleWeightedDiGraph())
